...
const {unlinkFile} = require ('./utils');

const exifFile = path.join (__dirname, 'bin', 'exiftool');
...
class ExifTool {
   ...
   *
   * param0: {
   *  source: image path || buffer of image
   *  tags: tags to get || get all tags
    ...
    return new Promise ((resolve, reject) => {
      process.nextTick (() => {
        ...
        exif.once ('close', code => {
          if (code === 0) {
            /**
             * giving filename will return filename
             */
            ...
            /**
             * giving buffer will return new buffer
             */
   ...
   *
   * @param {Array} tags New tag => value format that gives for add exif / iptc data into image
   */
